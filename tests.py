...
with Timer():
    list(parmap.parmap(sleep,range(10),N=2))
with Timer():
    list(parmap.parmap(sleep,range(10),N=4))

print('--- Set KW fixed ---')
# Set the elapsed time. From now on, use parmap with N=1
for N in [1,2,4]:
    with Timer():
        list(parmap.parmap(sleep,range(10),N=N,kwargs={'t':0.01}))

print('--- Set KW as args fixed ---')
# Set the elapsed time. From now on, use parmap with N=1
for N in [1,2,4]:
    with Timer():
        list(parmap.parmap(sleep,range(10),N=N,args=(0.01,) ))

print('--- Set KW varied ---')
# Set the elapsed time. From now on, use parmap with N=1
for N in [1,2,4]:
    with Timer():
        list(parmap.parmap(sleep,((i,{'t':i/40.0}) for i in range(10)),N=N,
                           kwstar=True))
print('--- Set KW as arg varied ---')
# Set the elapsed time. From now on, use parmap with N=1
for N in [1,2,4]:
    with Timer():
        list(parmap.parmap(sleep,((i,i/40.0) for i in range(10)),N=N,
                           star=True))
...
for N in [1,2,4]:
    with Timer():
        r = list(parmap.parmap(addsleep,zip(range(10),range(10)),
                               N=N,star=True))
        assert r == rgold
...
for N in [1,2,4]:
    with Timer():
        ...
        r = list(parmap.parmap(addsleep,zip(args,kwargs),
...
for N in [1,2,4]:
    with Timer():
        ...
        r = list(parmap.parmap(addsleep,args,
                               N=N,star=True))
        assert r == rgold
...
for N in [1,2,4]:
    with Timer():
        list(parmap.parmap(sleep,range(10),N=1,Nt=N,kwargs={'t':0.01}))

print('--- Lambdas ---')
# Make sure this (a) runs and (b) returns properly
assert list(parmap.parmap(lambda a:a,range(10),N=2)) == list(range(10))
assert list(parmap.parmap(lambda a,b:a+b,zip(range(10),range(10)),
                         star=True,N=2)) == [2*a for a in range(10)]
assert list(parmap.parmap(lambda a,b=2:a+b,range(10),
                          N=2)) == [a+2 for a in range(10)]
assert list(parmap.parmap(lambda a,b=2:a+b,range(10),
                          N=2,kwargs={'b':3})) == [a+3 for a in range(10)]
assert list(parmap.parmap(lambda a,b=2:a+b,zip(range(10),
                                              [{'b':i} for i in range(10)]),
                          N=2,kwstar=True)) == [2*a for a in range(10)]
assert list(parmap.parmap(lambda a,b,c=2:a+b+c,zip(zip(range(10),range(10)),
